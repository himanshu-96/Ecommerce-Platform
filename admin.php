<?php
	...
	if(!isset($_SESSION['admin'])){
		header("Location: ../libaaz2/adminlogin.php");
	}
?>
<!DOCTYPE html>
<html lang="en">
	...
	<nav class="navbar navbar-default navbar-fixed-top" role="navigation">
        <div class="container" style="width:100%">
            <div class="navbar-header page-scroll">
                ...
                <a class="navbar-brand page-scroll" href="#page-top">Administrator</a>
            </div>
...
<?php
	...
	if(isset($_SESSION['admin'])){
		$sql="SELECT * FROM administrator_name INNER JOIN administrator_private ON administrator_name.admin_id=administrator_private.admin_id WHERE administrator_name.admin_id='".$_SESSION['admin']."'";
		$result=$conn->query($sql);
		$row=$result->fetch_assoc(); 
		echo '          <ul class="dropdown-menu dropdown-user">
							<li><a href="#"><strong>'.$row['admin_name'].'</strong></a></li>
                            <li><a href="phpscripts/logoutadmin.php">Sign Out</a>
                            </li>
                        </ul>';
	}
?>
        	...
        	<div class="navba-default sidebar" role="navigation">
            	<div class="sidebar-nav navba-collapse">
                	<ul class="nav" id="side-menu">
                        ...
                        <li>
                            <a href="#wrapper4"><i class="fa fa-dashboard fa-fw"></i>Product Upload<span class="fa arrow"></span></a>
                        </li>
                        <li>
            ...
            <div class="row">
                <div class="col-md-8 col-lg-8 col-xs-12" style="height:100%;">
                    <div class="panel panel-default">
                        ...
                        <!-- /.panel-heading -->
                        <div class="panel-body chat" style="height:700px;overflow-y: scroll;">
                            <div id="product_edit">
<?php
		$conn=mysqli_connect("localhost", "root", "", "libaaz", "3306");
		if(!$conn)
		...
		$output .='	<div class="row">
						<div class="col-md-9 col-md-offset-1 col-sm-8 col-xs-6">
		...
		{
			$output .='<div class="col-md-3 col-sm-4 col-xs-6 img-thumbnail">
							<div class="col-md-12 col-xs-12 col-sm-12">Id : '.$row['product_id'].'</div>
...
?>                 
    ...
    <!---------->
    <div id="wrapper2" style="padding-top:50px;">
    	<div id="page-wrap">
        	<div class="row">
            	<div class="col-lg-12">
                	<h1 class="page-header">Users & Seller Details</h1>
                </div>
            </div>
            <!-- /.row -->
            <div class="row">
...
<?php
		...
		$output='';
		$sql="Select * from user_name INNER JOIN user_private ON user_name.user_id=user_private.user_id";
		$result=$conn->query($sql);
		while($row=mysqli_fetch_assoc($result))
		{
			$output .='<tr>
...
?>                               
                ...
                <div class="col-md-6 col-xs-12" style="height:100%;">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <i class="fa fa-bar-chart-o fa-fw"></i>Selllers
                            <div class="pull-right">
                            </div>
                        </div>
                        <!-- /.panel-heading -->
...
<?php
		...
		$output='';
		$sql="Select * from seller_name INNER JOIN seller_private ON seller_name.seller_id=seller_private.seller_id";
		$result=$conn->query($sql);
		while($row=mysqli_fetch_assoc($result))
		{
			$output .='<tr>
...
<?php
		...
		while($row=mysqli_fetch_assoc($result))
		{
			$output .='<tr>
                            ...
                            <td>'.$row['XL'].'</td>
                            <td>'.$row['XXL'].'</td>
							<td><input type="checkbox" id="'.$row['product_id'].'" value="'.$row['product_id'].'" name="quantity"/></td>
					   </tr>';
		}
		$output .='</table>';
		echo $output;
?>                 
    ...
    <!------>
    <div id="wrapper4" style="padding-top:50px;">
    	<div id="page-wrap">
        	<div class="row">
            	<div class="col-lg-12">
...
$(document).on("change", "input[name=quantity]", function(e) {
		...
		var checked1 = $('input[name=quantity]:checked');
		var checkedValues1 = checked1.map(function(i) { return $(this).val() }).get();
		var product_id = JSON.stringify(checkedValues1);
					
			$.ajax({
				method:"POST",
				url:"phpscripts/quantity.php",
				data:{product_id:product_id},
				success: function(data){
					$('#quantity').html(data);
				}
